...
class SelectorBuilder {
  ...
  element(value) {
    if (this.selectors.some((v) => /^[a-z0-9]+$/g.test(v))) {
      throw Error(
        'Element, id and pseudo-element should not occur more then one time inside the selector',
      );
    }
    if (
      this.selectors.some((v) => /^#[a-z0-9]+$/g.test(v))
      || this.selectors.some((v) => /^\.[a-z0-9]+$/g.test(v))
      || this.selectors.some((v) => /^\[.+\]$/g.test(v))
      || this.selectors.some((v) => /^:[a-z0-9]+$/g.test(v))
      || this.selectors.some((v) => /^::[a-z0-9]+$/g.test(v))
    ) {
      throw Error(
  ...
  combine(selector1, combinator, selector2) {
    this.selectors.push(selector1.selectors.join(''));
    this.selectors.push(` ${combinator} `);
    this.selectors.push(selector2.selectors.join(''));
    return this;
  }
}
